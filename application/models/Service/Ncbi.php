 ...
 * @author brady
 */
class Application_Model_Service_Ncbi
{
    ...
    public static function getGeneHomologs($ncbiGeneId)
    {
        ...
        if ($proteinAcc !== null) {
            $database = 'NCBI';
            $homologs = Application_Model_Service_Ppod::getHomologs($database, $proteinAcc);
        ...
        foreach ($uniprotIds as $uniprotId) {
            $database = 'UniProtKB';
            $homologs = Application_Model_Service_Ppod::getHomologs($database, $uniprotId);
            foreach ($homologs as $homolog) {
                $homolog['source'] = 'PPOD';
                $rows[] = $homolog;
            }
