...
$(document).ready(function() {
    ...
    $('.getGeneSymbol').click(function() {
        $(this).after(
            '<span class="getGeneSymbolIndicator loading-small" style="margin-left:5px"></span>'
        );
        ...
        var url = baseUrl + '/service/get-gene-symbol?'
            + 'ncbiGeneId=' + ncbiGeneId + '&'
            + 'targetId=' + targetId;
        $.get(url, function(data) {
            var dataObj = JSON.parse(data);
            $('#'+dataObj.targetId).val(dataObj.symbol);
            $('#'+dataObj.targetId).parent().parent().find(".getGeneSymbolIndicator").remove();
        });
        
        return false;
    });

    $('.getCompoundName').click(function() {
        $(this).after(
            '<span class="getCompoundNameIndicator loading-small" style="margin-left:5px"></span>'
        );

        var ncbiCompoundIdElement = $(this).prev('input');

        var elementId = ncbiCompoundIdElement.attr('id');
        var matches = elementId.match(/compounds-(\d+)-ncbiCompoundId/);
        var i = null;
        if (matches) {
            i = matches[1];
        }

        var targetId = 'compounds-' + i + '-name';
        var ncbiCompoundId = ncbiCompoundIdElement.val();
        var url = baseUrl + '/service/get-compound-name?'
            + 'ncbiCompoundId=' + ncbiCompoundId + '&'
            + 'targetId=' + targetId;
        $.get(url, function(data) {
            var dataObj = JSON.parse(data);
            $('#'+dataObj.targetId).val(dataObj.symbol);
            $('#'+dataObj.targetId).parent().parent().find(".getCompoundNameIndicator").remove();
        });
    ...
    $('#getCitationData').click(function() {
		...
		$('#getCitationData').after(
            '&nbsp;<span id="getCitationIndicator" class="loading-small"></span>'
        );

        var pubmedId = $('#citationPubmedId').val();
		$.getJSON(baseUrl + "/service/get-citation-data", { pubmedId: pubmedId }, function(data) {
            for (key in data) {
                if (key == 'year') {
    ...
    $('#getSpecies').click(function() {
		$('#getSpecies').after(
            '&nbsp;<span id="getSpeciesIndicator" class="loading-small"></span>'
        );

        var ncbiTaxonId = $('#ncbiTaxId').val();
		$.getJSON(baseUrl+"/service/get-species", { ncbiTaxonId: ncbiTaxonId }, function(data) {
            $('#species').val(data);
            $('#getSpeciesIndicator').remove();
            return false;
        });
        return false;
	});
